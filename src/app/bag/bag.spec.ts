...
import { newEvent, click } from '../../testing/utils';
...
describe('use inject helper in beforeEach', () => {
	...
	it('test should wait for FancyService.getObservableDelayValue', async () => {
		const value = await service.getObservableDelayValue().toPromise();
		expect(value).to.equal('observable delay value');
	});
});
...
describe('use inject within `it`', () => {
	...
	it('should use modified providers',
		inject([FancyService], (service: FancyService) => {
			...
			expect(service.getValue()).to.equal('value modified in beforeEach');
		})
	);

	it('test should wait for FancyService.getTimeoutValue',
		inject([FancyService], async (service: FancyService) => {
			const value = await service.getTimeoutValue();
			expect(value).to.equal('timeout value');
		}));
});

describe('using async/await/inject within beforeEach', () => {
	let serviceValue: string;

	beforeEach(async () => {
		TestBed.configureTestingModule({ providers: [FancyService] });
		await inject([FancyService], async (service: FancyService) => {
			serviceValue = await service.getAsyncValue();
		})();
	});

	it('should use asynchronously modified value ... in synchronous test', () => {
		expect(serviceValue).to.equal('async value');
	});
});
...
describe('TestBed Component Tests', () => {
	...
	it('should create a component with external template', () => {
		...
		expect(fixture).nodeText('from external template');
	});
	...
	it('should allow changing members of the component', () => {
		...
		fixture.detectChanges();
		expect(fixture).nodeText('MyIf()');

		fixture.componentInstance.showMore = true;
		fixture.detectChanges();
		expect(fixture).nodeText('MyIf(More)');
	});
	...
	it('should create a nested component bound to inputs/outputs', () => {
		...
		expect(heroes).to.have.lengthOf.above(0, 'has heroes');
		...
		expect(selected).nodeText(hero.name);
	});
	...
	it('can access the instance variable of an `*ngFor` row component', () => {
		...
		expect(hero.name).to.equal(heroName, 'ngRow.context.hero');

		const rowComp = ngForRow.componentInstance;
		// jasmine.any is an "instance-of-type" test.
		expect(rowComp).to.be.instanceOf(IoComponent, 'component is IoComp');
		expect(rowComp.hero.name).to.equal(heroName, 'component.hero');
	});
	...
	it('should support clicking a button', () => {
		...
		fixture.detectChanges();
		expect(span.textContent).to.match(/is off/i, 'before click');

		click(btn);
		fixture.detectChanges();
		expect(span.textContent).to.match(/is on/i, 'after click');
	});
	...
	it('should support entering text in input box (ngModel)', async () => {
		...
		await fixture.whenStable();
		expect(comp.name).to.equal(expectedNewName,
	...
	it('ReversePipeComp should reverse the input text', async () => {
		...
		await fixture.whenStable();
		fixture.detectChanges();
		expect(span.textContent).to.equal(expectedText, 'output span');
		expect(comp.text).to.equal(inputText, 'component.text');
	});
	...
	it('can examine attached directives and listeners', () => {
		...
		expect(inputEl.providerTokens).to.contain(NgModel, 'NgModel directive');

		const ngControl = inputEl.injector.get(NgControl);
		expect(ngControl).to.be.instanceOf(NgControl, 'NgControl directive');

		expect(inputEl.listeners).to.have.lengthOf.above(2, 'several listeners attached');
	});
	...
	it('BankAccountComponent should set attributes, styles, classes, and properties', () => {
		...
		expect(childComp).to.be.instanceOf(BankAccountComponent);

		expect(el.context).to.equal(childComp, 'context is the child component');
...
describe('TestBed Component Overrides:', () => {
	...
	it('should override TestProvidersComp\'s FancyService provider', () => {
		...
		fixture.detectChanges();
		expect(fixture).nodeText('injected value: faked value', 'text');

		// Explore the providerTokens
		const tokens = fixture.debugElement.providerTokens;
		expect(tokens).to.contain(fixture.componentInstance.constructor, 'component ctor');
		expect(tokens).to.contain(TestProvidersComponent, 'TestProvidersComp');
		expect(tokens).to.contain(FancyService, 'FancyService');
	});
	...
	it('should override TestViewProvidersComp\'s FancyService viewProvider', () => {
		...
		fixture.detectChanges();
		expect(fixture).nodeText('injected value: faked value');
	});
...
describe('Nested (one-deep) component override', () => {
	...
	it('ParentComp should use Fake Child component', () => {
		...
		fixture.detectChanges();
		expect(fixture).nodeText('Parent(Fake Child)');
	});
});
...
describe('Nested (two-deep) component override', () => {
	...
	it('should use Fake Grandchild component', () => {
		const fixture = TestBed.createComponent(ParentComponent);
		fixture.detectChanges();
		expect(fixture).nodeText('Parent(Fake Child(Fake Grandchild))');
	});
});
...
describe('Lifecycle hooks w/ MyIfParentComp', () => {
	...
	it('child component called OnChanges once', () => {
		fixture.detectChanges();
		getChild();
		expect(child.ngOnChangesCounter).to.equal(1);
	});
	...
	it('changed parent value flows to child', () => {
		...
		expect(child.ngOnChangesCounter).to.equal(2,
			'expected 2 changes: initial value and changed value');
		expect(child.childValue).to.equal('foo',
	...
	it('changed child value flows to parent', async () => {
		...
		expect(child.ngOnChangesCounter).to.equal(2,
			'expected 2 changes: initial value and changed value');
		expect(parent.parentValue).to.equal('bar',
